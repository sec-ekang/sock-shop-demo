...
module "iam" {
  source              = "../modules/iam"
  cluster_role_name   = "prod-${var.cluster_role_name}"
  cluster_policy_arns = var.cluster_policy_arns
  node_role_name      = "prod-${var.node_role_name}"
  node_policy_arns    = var.node_policy_arns
}

module "eks" {
  source              = "../modules/eks"
  cluster_name        = "prod-${var.cluster_name}"
...
module "jenkins" {
  ...
  name              = "prod-jenkins-instance"
}

module "rds" {
  source              = "../modules/rds"
  subnet_group_name   = "prod-${var.rds_subnet_group_name}"
  subnet_ids          = [module.vpc.private_subnets_ids[0]]
  db_identifier       = "prod-${var.rds_db_identifier}"
...
module "ecr_sockshop_carts" {
  source          = "../modules/ecr"
  repository_name = "prod-${var.ecr_repository_name_carts}"
  environment     = "prod"
}

module "ecr_sockshop_orders" {
  source          = "../modules/ecr"
  repository_name = "prod-${var.ecr_repository_name_orders}"
  environment     = "prod"
}

module "cloudwatch" {
  source            = "../modules/cloudwatch"
  log_group_name    = "/aws/eks/prod-${var.cloudwatch_log_group_name}"
