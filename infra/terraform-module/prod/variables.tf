...
variable "vpc_name" {
  description = "Base name of the VPC (without environment prefix)"
  type        = string
...
variable "eks_nodes_cidr_blocks" {
  description = "CIDR blocks for EKS node group access"
  type        = list(string)
...
variable "jenkins_ami_id" {
  ...
  type        = string
  default     = "ami-0c55b159cbfafe1f0"  # Update with a Prod AMI if different
}
...
variable "rds_password" {
  ...
  type        = string
  sensitive   = true
  default     = "prodpassword"  # Replace with secure secret for Prod
}
